...
parser.add_argument('-imgf','--imageFolder', required=True, type=str)
parser.add_argument('-mp', '--modelPath', required=True ,type=str)
...
stripped = args['modelPath'].split(path_sep, 1)[1]
...
elif "STYLEGAN2" in args["imageFolder"]:
    data_name = "STYLEGAN2"

elif "celeba-hq_real_aligned" in args["imageFolder"]:
    data_name = "celeba-hq_real_aligned"

elif "ffhq_aligned" in args["imageFolder"]:
    data_name = "ffhq_aligned"

elif "stargan_aligned" in args["imageFolder"]:
    data_name = "stargan_aligned"

elif "progan_aligned" in args["imageFolder"]:
    data_name = "progan_aligned"

elif "stylegan3-0.5_aligned" in args["imageFolder"]:
    data_name = "stylegan3-0.5_aligned"

elif "stylegan-ada-0.5_aligned" in args["imageFolder"]:
    data_name = "stylegan-ada-0.5_aligned"

#elif "original_data" in args["imageFolder"]:
...
cam_directory = "/scratch365/ccrum/Cyborg/camdeg-salt-v2-cams/" + args['gradcamType'] + "/"  + data_name + "/" + folder + "/" + args["size"] + "/" #+ degrad_type
...
for img_path in glob.glob(directory):
    try:
        ...
        save_directory = str(cam_directory) + "/" + str(image_name)
