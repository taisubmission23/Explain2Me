...
output_file = "./results-v2/" + dataset_name + "-" + model_training_name + "-" + args["type"] + "-" + ".txt"
...
save_directory = "/scratch365/ccrum/Cyborg/camdeg-salt-v2/" + data_name + "/" + args['type'] + "/"  + args["network"] + "/"  + model_training_name + "/"
if not os.path.exists(save_directory):
    os.makedirs(save_directory)

levels = ['0.00', '0.01', '0.05', '0.10', '0.25', '0.50', '0.75', '1.0']
...
for image_file in resample(glob.glob(directory), n_samples=10000, replace=True, random_state=1):
    ...
    for level in levels:
        ...
        salt_pepper = skimage.util.random_noise(image, mode="s&p", amount=float(level))
        salt_pepper = np.array((salt_pepper * 255).astype(np.uint8))
        salt_pepper = Image.fromarray(salt_pepper)

        save_dir = save_directory + str(level) + "-" + image_name
        ...
        data = str(level) + "," + str(image_name)  + "," + str(PAScore) + "," + str(model_training_name) + "," + str(args["network"]) + "," + str(dataset_name) + "," + str(args["type"])
        print(data)
